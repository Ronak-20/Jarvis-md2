...
async (message, match) => {
    if (!match)
        return await message.sendPollMessage({ name: "Choose mod to change mod", values: [["private", "mode private"], ["public", "mode public"]], withPrefix: true, participates: [message.sender] });
   
if (!match === "private" || !match === "public") 
    return await message.sendPollMessage({ name: "Choose mod to change mode", values: [["private", "mode private"], ["public", "mode public"]], withPrefix: true, participates: [message.sender] });
